...
public class ItemHandlerTest {
  ...
  @Autowired
  public ItemHandlerTest(WebTestClient webTestClient, ItemReactiveRepository items) {
  ...
  public void getAll_Flux() {
    Flux<Item> itemFlux = webTestClient.get().uri("/functional/items")
        .exchange()
        .expectStatus().isOk()
        .expectHeader().contentType(MediaType.APPLICATION_JSON)
  ...
  @Test
  public void getOne() {
    webTestClient.get().uri("/functional/items/{id}", "predefinedId")
        .exchange()
        .expectStatus().isOk()
        .expectBody()
  ...
  public void getOne_notFound() {
    webTestClient.get().uri("/functional/items/{id}", "wrongId")
        .exchange()
  ...
  public void update() {
    ...
    webTestClient.put().uri("/functional/items/{id}", "predefinedId")
        .contentType(MediaType.APPLICATION_JSON)
  ...
  public void update_wrongId() {
    final double discountPrice = 399.99;
    webTestClient.put().uri("/functional/items/{id}", "wrongId")
        .contentType(MediaType.APPLICATION_JSON)
        .accept(MediaType.APPLICATION_JSON)
        .body(
            Mono.just(
  ...
  @Test
  public void delete() {
    webTestClient.delete().uri("/functional/items/{id}", "predefinedId")
        .exchange()
        .expectStatus().isOk()
